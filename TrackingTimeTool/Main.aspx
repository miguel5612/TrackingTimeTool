...
<h1> Bienvenido <asp:Label ID="Email" runat="server" />!</h1>
 <div class="">
     ...
     <div id="hiddenInsert">
     <asp:FormView ID="FormView1" runat="server" DefaultMode="Insert" RenderOuterTable="true" DataKeyNames="IDWork" DataSourceID="TimesFormView">
         <EditItemTemplate>
             IDWork:
         ...
         <InsertItemTemplate>
             ...
             <asp:TextBox ID="endDateTextBox" runat="server" Text='<%# Bind("endDate") %>' />
             <br />
             <asp:LinkButton ID="InsertButton" ClientIDMode="Static" runat="server" CausesValidation="True" CommandName="Insert" Text="Insert" />
     ...
     <asp:SqlDataSource ID="TimesFormView" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:TTTConectionString %>" DeleteCommand="DELETE FROM [Works] WHERE [IDWork] = @original_IDWork AND [IDUser] = @original_IDUser AND [IDProyecto] = @original_IDProyecto AND (([WorkTitle] = @original_WorkTitle) OR ([WorkTitle] IS NULL AND @original_WorkTitle IS NULL)) AND (([WorkDate] = @original_WorkDate) OR ([WorkDate] IS NULL AND @original_WorkDate IS NULL)) AND [CreationDate] = @original_CreationDate AND (([Bemerkungen] = @original_Bemerkungen) OR ([Bemerkungen] IS NULL AND @original_Bemerkungen IS NULL)) AND (([endDate] = @original_endDate) OR ([endDate] IS NULL AND @original_endDate IS NULL))" InsertCommand="INSERT INTO [Works] ([IDUser], [IDProyecto], [WorkTitle], [WorkDate], [CreationDate], [Bemerkungen], [endDate]) VALUES (@IDUser, @IDProyecto, @WorkTitle, @WorkDate, @CreationDate, @Bemerkungen, @endDate)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT * FROM [Works] WHERE ([IDUser] = @IDUser)" UpdateCommand="UPDATE [Works] SET [IDUser] = @IDUser, [IDProyecto] = @IDProyecto, [WorkTitle] = @WorkTitle, [WorkDate] = @WorkDate, [CreationDate] = @CreationDate, [Bemerkungen] = @Bemerkungen, [endDate] = @endDate WHERE [IDWork] = @original_IDWork AND [IDUser] = @original_IDUser AND [IDProyecto] = @original_IDProyecto AND (([WorkTitle] = @original_WorkTitle) OR ([WorkTitle] IS NULL AND @original_WorkTitle IS NULL)) AND (([WorkDate] = @original_WorkDate) OR ([WorkDate] IS NULL AND @original_WorkDate IS NULL)) AND [CreationDate] = @original_CreationDate AND (([Bemerkungen] = @original_Bemerkungen) OR ([Bemerkungen] IS NULL AND @original_Bemerkungen IS NULL)) AND (([endDate] = @original_endDate) OR ([endDate] IS NULL AND @original_endDate IS NULL))">
         <DeleteParameters>
